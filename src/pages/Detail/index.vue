      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item, index) in spuSaleAttrList" :key="index">
                ...
                <dd
                  ...
                  v-for="(color, cindex) in item.spuSaleAttrValueList"
                  :key="cindex"
                  @click="chooseAttri(color,item.spuSaleAttrValueList)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="goodNum" @change="putGoodNum"/>
                <a href="javascript:" class="plus" @click="goodNum++">+</a>
                <a href="javascript:" class="mins" @click="goodNum<=1?goodNum=1:goodNum--">-</a>
              </div>
              <div class="add">
                <a href="javascript:" @click="addCar">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods:{
    ...
    //购物车商品数量
    putGoodNum(event){
      let goodnum = event.target.value
      if(isNaN(goodnum) || goodnum < 1){
        this.goodNum = 1
      }else{
    ...
    async addCar(){
      try{
        const result = await this.$store.dispatch('addGoodCard',{skuID:this.$route.params.goodid,skuNum:this.goodNum})
        //将商品信息会话存储到本地浏览器
        sessionStorage.setItem("GoodInfo",JSON.stringify(this.skuInfo))
        this.$router.push({name:"addgoodcard",query:{num:this.goodNum}})    
      }
      catch(error){
  ...
  mounted() {
    this.$store.dispatch("getDetail", this.$route.params.goodid);
  },
};
</script>
